...
#the outputs of this script are the inla model results for the areas getting wetter  
...
###Select those getting wetter over time###
#reduce dataset to those that are getting wetter from the initial surface water state
#these pixels that are more than 3 which denote the surface water categories - rarely flooded (3) or never flooded (4)
#then converted into a true or false statement 
cov_spdf_wetter <- cov_spdf[cov_spdf$initial_sw >2,]

#selects those pixels that have been getting wetter over time
#creates a new column sig_wet which gives a 0 or 1 then a T or F 
cov_spdf_wetter$sig_wet <- (cov_spdf_wetter$sw_transition >= 2) * 1 
...
family = 'binomial' #regression family, rate of change normal
...
res <- inla(formula, 
            ...
            control.predictor=list(A = inla.stack.A(stck), 
            ...
            #control.family = control.family, #remove for binomial model, don't need 
...
save(res, file = 'inla_model_results_wetter.Rdata')
...
plot(xs, res$summary.lincomb.derived$"0.5quant"[grep("roads", rownames(res$summary.lincomb.derived))], type = "n", axes = F, bty = "l", 
     ...
     ylab = "Logit prob of extreme increase of flooding", pch = 20, col = rgb(0,0,0,0.05),
     log = "x")
...
plot(c(1,3), c(range(res$summary.lincomb.derived[grep("landuse", rownames(res$summary.lincomb.derived)), 4:6])), 
     ...
     xaxt = "n",
     xlab = "Land Use",
     ylab = "Logit prob of extreme increase of flooding")
