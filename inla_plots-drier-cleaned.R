...
#it creates a rug plot to show the logit probability of extreme drying with distance to roads
#and it also creates a forest plot to show the logit probability of extreme drying and presence of different landuses 
...
#those that are less than 3, irreg dry or perm dry (1s and 2s)
#converted into a true or false statement 
cov_spdf_drier <- cov_spdf[cov_spdf$initial_sw <3,]

#selects those squares that have been drying over time
#creates a new column sig_dry - -gives a 0 or 1 then a T or F 
cov_spdf_drier$sig_dry <- (cov_spdf_drier$sw_transition <= -2) * 1 #* 1 converts a logical true or false then converted to 1 or 0 so that INLA can understand it 
...
#and the spde: applying the mesh to the locations, linking it --> a structure (A)
A = inla.spde.make.A(mesh=mesh, loc=data.matrix(coordinates(cov_spdf_drier)))  # Combine actual points with mesh
...
stck <- inla.stack(tag='est',     # stack is INLA data structure
                   ...
                   effects=list(#lists the Model Components
                     s=1:mesh$n,# - The "s" is means "spatial"
...
#Ffunction to add letter to top left corner of each of the plots for reference 
...
plot(xs, res$summary.lincomb.derived$"0.5quant"[grep("roads", rownames(res$summary.lincomb.derived))], type = "n", axes = F, bty = "l", 
     ...
     xlim = c(400, max(cov_spdf_drier$roads)),  
      ...
      ylab = "Probability of extreme drying", pch = 20, col = rgb(0,0,0,0.05),
      log = "x")
...
#100 lines that represent the density of the points in the raw data, ones that didn't get drier
rug(quantile(cov_spdf_drier$roads[cov_spdf_drier$sig_dry == 0], seq(0, 1, 0.01)))

#those points that did get drier, adds this to the top of the plot
#pixels that are closer to the roads, as closer to the roads more likely of a drying event
...
plot(c(1,3), c(range(res$summary.lincomb.derived[grep("landuse", rownames(res$summary.lincomb.derived)), 4:6])), 
        ...
        xaxt = "n",
        xlab = "Land Use",
        ylab = "Probability of extreme drying")
axis(1, at = c(1.25, 2, 2.75), labels = c("Other","Economic Land Concessions", "Protected Areas"))
for(i in 1:3){
